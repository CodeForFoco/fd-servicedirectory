...
test("formatService", () => {
  const service = [
    ...
    1,
    2,
    "Food, clothing, transit, and housing",
    4,
    5,
    "Y",
    "Y",
...
test("getRequirementName", () => {
  ...
  Object.keys(expected).forEach(key => {
    expect(utils.getRequirementName({ name: key, format: "expanded" })).toEqual(
      expected[key].expanded
    );
    expect(utils.getRequirementName({ name: key, format: "compact" })).toEqual(
